...
- Les photos envoyées (Jeu de Paume, Petit Palais, Palais de Tokyo, Musée Picasso, Fondation Louis Vuitton, Centre Pompidou) sont ajoutées à l'application.
...
Six photos sont disponibles pour l'instant. Il manque le Musée d'Orsay et le Louvre ; deux options :
...
- Photos envoyées via `lovable-assets` → pointeurs dans `src/assets/museums/` (`jeu-de-paume`, `petit-palais`, `palais-de-tokyo`, `musee-picasso`, `fondation-louis-vuitton`, `centre-pompidou`).
